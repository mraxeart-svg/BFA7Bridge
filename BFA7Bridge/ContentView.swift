        ...
        NavigationStack {
            List {
                ...
                Section("Подключение") {
                    ...
                    Button("Сбросить BLE-подключение") {
                        glasses.resetConnection()
                    }
...
private struct SVAuthLabSection: View {
    ...
    private func reportText(_ build: BFA7SVCommandBuild) -> String {
        ...
        lines.append("- APK SV BLE profile: service AD3072F9-DCCB-4A10-989F-CA7EE37AB757, notify 00001801..., write 00001802..., read 00001800....")
        lines.append("- FE95/005E/005F is Xiaomi MiWear/SAR traffic; it is not the confirmed SV command path.")
        lines.append("")
        lines.append("Notes:")
        lines.append(contentsOf: build.notes.map { "- \($0)" })
        lines.append("")
