/*
 * BFA7 / Xiaomi Glasses iOS STARTUP HANDSHAKE hook.
 *
 * No Swift Interceptor.attach. This avoids the crash seen when attaching to
 * MIWFlowEncrypt.encrypt(data:). The hook:
 ...
 * - hooks ObjC CoreBluetooth writes, notify subscription, incoming FE95 notify, and Hotspot config
 *
 * Run:
 *   py -3.12 -m frida_tools.repl -H 127.0.0.1:27042 -p <PID> -l frida-ios-xiaomi-startup-handshake-hook.js | Tee-Object <log>
 */
...
function log(message) { console.log(`[BFA7-iOS-HANDSHAKE ${now()}] ${message}`); }
...
function hookCoreBluetooth() {
  hookObjCMethod('CBPeripheral', '- writeValue:forCharacteristic:type:', {
    onEnter(args) {
      ...
      if (characteristic.indexOf('005F') < 0 && characteristic.indexOf('005E') < 0 && length < 8) return;
  ...
  hookObjCMethod('CBPeripheral', '- setNotifyValue:forCharacteristic:', {
    onEnter(args) {
      const characteristic = characteristicSummary(args[3]);
      if (characteristic.indexOf('005F') < 0 && characteristic.indexOf('005E') < 0 && characteristic.indexOf('FE95') < 0) return;
      log(`CB SET_NOTIFY enabled=${safe(() => args[2].toInt32(), '?')} characteristic=${characteristic}`);
    }
  });
...
function hookCBDelegateNotifications() {
  ...
  Object.keys(ObjC.classes).forEach(className => {
    if (!classTerms.test(className)) return;
    const cls = ObjC.classes[className];
    ...
    try {
      Interceptor.attach(method.implementation, {
        onEnter(args) {
          ...
          log(`CB NOTIFY delegate=${className} characteristic=${characteristic} ${nsDataInfo(valuePtr, 1024)}`);
          scheduleHeapProbe('after-cb-notify');
        }
      });
...
function install() {
  ...
  log('Installing BFA7 iOS Xiaomi STARTUP HANDSHAKE hook');
  hookCoreBluetooth();
...
rpc.exports = {
  ping() { return `handshake hooks=${STATE.hooked.size} classes=${STATE.classNames.length}`; },
