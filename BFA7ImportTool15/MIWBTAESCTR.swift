import Foundation

enum MIWBTAESCTRError: Error {
    case invalidKeyLength
    case invalidCounterLength
}

enum MIWBTAESCTR {
    static func encrypt(_ plaintext: Data, key: Data, initialCounter: Data) throws -> Data {
        guard key.count == 16 else { throw MIWBTAESCTRError.invalidKeyLength }
        guard initialCounter.count == 16 else { throw MIWBTAESCTRError.invalidCounterLength }

        let roundKeys = expandKey(Array(key))
        var counter = Array(initialCounter)
        var output = [UInt8]()
        output.reserveCapacity(plaintext.count)

        let bytes = Array(plaintext)
        var index = 0
        while index < bytes.count {
            let stream = encryptBlock(counter, roundKeys: roundKeys)
            let count = min(16, bytes.count - index)
            for offset in 0..<count {
                output.append(bytes[index + offset] ^ stream[offset])
            }
            incrementBigEndian(&counter)
            index += count
        }

        return Data(output)
    }

    private static func incrementBigEndian(_ counter: inout [UInt8]) {
        for index in stride(from: counter.count - 1, through: 0, by: -1) {
            let (value, overflow) = counter[index].addingReportingOverflow(1)
            counter[index] = value
            if !overflow { break }
        }
    }

    private static func expandKey(_ key: [UInt8]) -> [UInt8] {
        var expanded = key
        var rconIndex = 1
        while expanded.count < 176 {
            var temp = Array(expanded[(expanded.count - 4)..<expanded.count])
            if expanded.count % 16 == 0 {
                temp = [temp[1], temp[2], temp[3], temp[0]]
                temp = temp.map { sBox[Int($0)] }
                temp[0] ^= rcon[rconIndex]
                rconIndex += 1
            }
            for item in temp {
                expanded.append(expanded[expanded.count - 16] ^ item)
            }
        }
        return expanded
    }

    private static func encryptBlock(_ block: [UInt8], roundKeys: [UInt8]) -> [UInt8] {
        var state = block
        addRoundKey(&state, roundKeys, round: 0)
        for round in 1..<10 {
            subBytes(&state)
            shiftRows(&state)
            mixColumns(&state)
            addRoundKey(&state, roundKeys, round: round)
        }
        subBytes(&state)
        shiftRows(&state)
        addRoundKey(&state, roundKeys, round: 10)
        return state
    }

    private static func addRoundKey(_ state: inout [UInt8], _ roundKeys: [UInt8], round: Int) {
        let start = round * 16
        for index in 0..<16 {
            state[index] ^= roundKeys[start + index]
        }
    }

    private static func subBytes(_ state: inout [UInt8]) {
        for index in 0..<state.count {
            state[index] = sBox[Int(state[index])]
        }
    }

    private static func shiftRows(_ state: inout [UInt8]) {
        let original = state
        for row in 1..<4 {
            for column in 0..<4 {
                state[row + (4 * column)] = original[row + (4 * ((column + row) % 4))]
            }
        }
    }

    private static func mixColumns(_ state: inout [UInt8]) {
        for column in 0..<4 {
            let index = column * 4
            let a0 = state[index]
            let a1 = state[index + 1]
            let a2 = state[index + 2]
            let a3 = state[index + 3]
            state[index] = multiply2(a0) ^ multiply3(a1) ^ a2 ^ a3
            state[index + 1] = a0 ^ multiply2(a1) ^ multiply3(a2) ^ a3
            state[index + 2] = a0 ^ a1 ^ multiply2(a2) ^ multiply3(a3)
            state[index + 3] = multiply3(a0) ^ a1 ^ a2 ^ multiply2(a3)
        }
    }

    private static func multiply2(_ value: UInt8) -> UInt8 {
        let shifted = UInt8(truncatingIfNeeded: UInt16(value) << 1)
        return (value & 0x80) == 0 ? shifted : shifted ^ 0x1B
    }

    private static func multiply3(_ value: UInt8) -> UInt8 {
        multiply2(value) ^ value
    }

    private static let rcon: [UInt8] = [
        0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36
    ]

    private static let sBox: [UInt8] = [
        
    ]
}
